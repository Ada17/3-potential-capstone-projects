...
#titanic_orig[, titanic_orig$embarked][is.na(titanic_orig[, titanic_orig$embarked])] <- "S"
titanic_orig[c(titanic_orig$embarked)][is.na(titanic_orig[c(titanic_orig$embarked)])] <- "S"
#xtit <- c(titanic_orig$embarked)
#xtit[is.na(xtit)] <- "S"
#print(paste("testing xtit ", xtit))
...
for(m in 1:length(titanic_orig$cabin)){
 ...
 #write.csv(titanic_origupd, file = "C:/users/aalutu/Desktop/work/pers/data science/titan_clean.csv")
